...
export async function GET(req: NextRequest, ctx: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  // Use Invidious to obtain audio-only stream (itag 140 m4a)
  const src = `https://yewtu.be/latest_version?id=${id}&itag=140`;
  ...
  headers.set('Content-Disposition', `inline; filename="${id}.m4a"`);
